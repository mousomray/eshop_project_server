...
class adminAuthController {
    ...
    async registerPost(req, res) {
        try {
            ...
            if (!first_name || !last_name || !email || !password || !req.file) {
                req.flash('err', 'All fields are required including image');
            ...
            if (password.length < 8) {
                req.flash('err', 'Password should be atleast 8 chracters long');
                return res.redirect(generateUrl('register'));
            }
    ...
    async loginPost(req, res) {
        try {
            ...
            if (token) {
                ...
                return res.redirect(generateUrl('admindashboard'));
            } else {
    ...
    async RegList(req, res) {
        try {
            const registrations = await AuthRepo.registrationList();
            res.render('auth/registrationlist', { registrations, user: req.user })
        } catch (error) {
